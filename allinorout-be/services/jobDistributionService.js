...
import { sendJobAlertEmail, sendJobClaimedEmail, sendBookingCompletedEmail, sendBookingCompletedMoverEmail } from '../utils/email.js';
...
class JobDistributionService {
  ...
  async markJobCompleted(jobAlertId) {
    ...
    if (moverEmail) {
      try {
        await sendBookingCompletedMoverEmail(completionData);
        console.log('✅ Booking completion email sent to mover:', moverEmail);
      } catch (emailError) {
        console.error('❌ Failed to send booking completion email to mover:', emailError);
      }
    }
