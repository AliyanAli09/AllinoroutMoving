...
import { sendPasswordResetEmail, sendPasswordResetConfirmationEmail, sendWelcomeEmail, sendWelcomeMoverEmail } from '../utils/email.js';
...
export const registerMover = asyncHandler(async (req, res, next) => {
  ...
  // Send welcome email
  try {
    await sendWelcomeMoverEmail(mover);
  } catch (error) {
    console.error('Failed to send welcome email:', error);
    // Don't fail registration if email fails
  }
