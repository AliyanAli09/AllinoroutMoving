...
import { FiTruck, FiMapPin, FiCalendar, FiHome, FiClock, FiDollarSign, FiMessageSquare, FiCheck, FiX, FiArrowLeft, FiLoader } from 'react-icons/fi';
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm text-gray-600">
                          <div className="space-y-2">
                            <div className="flex items-start space-x-2">
                              ...
                              <span><strong>From:</strong> {alert.booking?.pickupAddress ? `${alert.booking.pickupAddress.city}, ${alert.booking.pickupAddress.state} ${alert.booking.pickupAddress.zipCode}` : 'N/A'}</span>
                            </div>
                            <div className="flex items-start space-x-2">
                              <FiMapPin className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400 mt-0.5 flex-shrink-0" />
                              <span><strong>To:</strong> {alert.booking?.dropoffAddress ? `${alert.booking.dropoffAddress.city}, ${alert.booking.dropoffAddress.state} ${alert.booking.dropoffAddress.zipCode}` : 'N/A'}</span>
                            </div>
                            <div className="flex items-start space-x-2">
                              <FiCalendar className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400 mt-0.5 flex-shrink-0" />
                              <span><strong>Move Date:</strong> {alert.booking?.moveDate ? new Date(alert.booking.moveDate).toLocaleDateString() : 'N/A'}</span>
                            </div>
                          </div>
                          <div className="space-y-2">
                            <div className="flex items-start space-x-2">
                              <FiHome className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400 mt-0.5 flex-shrink-0" />
                              <span><strong>Size:</strong> {alert.booking?.homeSize || 'N/A'}</span>
                            </div>
                            <div className="flex items-start space-x-2">
                            ...
                            <div className="flex items-start space-x-2">
                              ...
                              <span><strong>Job Payment:</strong> {alert.booking?.quote?.subtotal ? `$${alert.booking.quote.subtotal}` : 'N/A'}</span>
                            </div>
                          </div>
                        </div>
